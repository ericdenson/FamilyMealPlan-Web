    ...
    async fetchModerationQueue() {
        try {
            ...
            const { data, error } = await db.from('recipes')
                .select('id, title, author_id, visibility, instructions, is_duplicate, profanity_detected, duplicate_of_title, recipe_ingredients(ingredient_name), users!recipes_author_id_fkey(first_name)')
