...
const adminApp = {
    ...
    async fetchAdminTickets() {
        try {
            ...
            const openCommsCount = allTix.filter(t => t.status === 'open' && t.subject_tag !== 'RECIPE').length;
    ...
    async computePortalDashboard() {
        try {
            ...
            const { data: freshTickets } = await db.from('support_tickets').select('status, subject_tag');
            const openTickets = freshTickets ? freshTickets.filter(t => t.status === 'open' && t.subject_tag !== 'RECIPE').length : 0;
