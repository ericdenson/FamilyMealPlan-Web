...
const adminApp = {
        ...
        try {
            ...
            if (usersTbody) {
                ...
                recentSignups.forEach(u => {
                    ...
                    tr.innerHTML = `
                        <td><strong>${u.name || 'Anonymous User'}</strong><br><span style="font-size:0.8rem; color:#888;">${u.email}</span></td>
                        <td style="text-transform: capitalize;">${u.diet_preference || 'Standard'}</td>
                        <td>${statusHtml}</td>
        ...
        try {
            ...
            const { data, error } = await db.auth.signInWithOtp({
                ...
                options: {
                    ...
                    emailRedirectTo: window.location.origin + '/admin-portal.html'
                }
            });
    ...
    async generateNewImage() {
        ...
        try {
            // 1. Fetch AI Generated image Blob from Pollinations AI
            const encodedPrompt = encodeURIComponent(`Professional food photography of ${title}, highly detailed, 4k, studio lighting, highly appetizing, culinary masterpiece`);
            const aiImageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?nologo=true&width=800&height=600`;
