...
const adminApp = {
    ...
    async generateNewImage() {
        ...
        try {
            // 1. Call custom DALL-E 3 Edge Function
            const { data: edgeData, error: edgeError } = await db.functions.invoke('generate-image', {
                body: { title: title }
            });
            
            if (edgeError) throw new Error("AI Generation Edge Function failed: " + edgeError.message);
            if (!edgeData || !edgeData.url) throw new Error("AI Generation failed to return an image URL");
            
            const aiImageUrl = edgeData.url;
