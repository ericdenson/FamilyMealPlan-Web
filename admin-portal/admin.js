...
const adminApp = {
    ...
    previewModerationRecipe(recipeId) {
        ...
        dialog.innerHTML = `
            ...
            <div style="display:flex; gap:16px; justify-content:flex-end; border-top:1px solid #eee; padding-top:24px;">
                <button class="btn" style="background:#6b8b72; padding: 12px 24px; font-size: 1rem;" onclick="adminApp.executeModerationApprove('${recipe.id}', '${recipe.author_id}', document.getElementById('mod-message').value); this.closest('dialog').close(); this.closest('dialog').remove();"><i class="fa-solid fa-check"></i> Approve & Publish</button>
    ...
    async executeModerationApprove(recipeId, authorId, rawMessage) {
        const bodyContent = rawMessage ? rawMessage : 'Your recipe natively cleared the Moderation Queue and is now live in the global community cookbook!';
        try {
    ...
    addIngredientRow(name = "", category = "Pantry & Dry Goods") {
        ...
        div.style.gap = "8px";
        div.innerHTML = `
            <input type="text" class="ig-name" value="${typeof name === 'string' ? name.replace(/"/g, '&quot;') : ''}" placeholder="e.g. 2 cups Rice" style="flex: 2; padding: 10px; border: 1px solid var(--border-light); border-radius: 6px;">
            <select class="ig-cat" style="flex: 1; padding: 10px; border: 1px solid var(--border-light); border-radius: 6px;">
                <option value="Produce" ${category === 'Produce' ? 'selected' : ''}>Produce</option>
                <option value="Meat & Seafood" ${category === 'Meat & Seafood' ? 'selected' : ''}>Meat & Seafood</option>
                <option value="Dairy & Eggs" ${category === 'Dairy & Eggs' ? 'selected' : ''}>Dairy & Eggs</option>
                <option value="Pantry & Dry Goods" ${category === 'Pantry & Dry Goods' ? 'selected' : ''}>Pantry & Dry</option>
    ...
    async saveRecipe() {
        ...
        try {
            ...
            igRows.forEach(row => {
                ...
                const c = row.querySelector('.ig-cat').value;
                if (n) finalIngredients.push({ ingredient_name: n, shopping_category: c });
            });
    ...
    async generateNewImage() {
        ...
        if (!title) {
            return alert("Must have a recipe title first!");
        }
        ...
        try {
            // 1. Get or Prompt for OpenAI API Key securely via browser Memory
            let apiKey = localStorage.getItem('openai_api_key');
            if (!apiKey) {
                apiKey = prompt("Please enter your OpenAI API Key to generate this photo. It will be securely saved only in your local browser cache.");
                if (!apiKey) throw new Error("API Key is required to fetch image.");
                localStorage.setItem('openai_api_key', apiKey.trim());
            }

            // 2. Call DALL-E 3 API directly from Admin Client
            const openAiRequest = await fetch("https://api.openai.com/v1/images/generations", {
                 method: "POST",
                 headers: {
                 ...
                 body: JSON.stringify({
                      model: "dall-e-3",
                      prompt: `Professional culinary food photography of ${title}, highly detailed, 4k, studio lighting, photorealistic.`,
                      n: 1,
                      size: "1024x1024"
                 })
            });
            const openAiData = await openAiRequest.json();
            if (!openAiRequest.ok) throw new Error("OpenAI Generation failed: " + (openAiData.error?.message || "Unknown error"));
            
            const aiImageUrl = openAiData.data[0].url;
            ...
            // 2. Upload Blob securely to Supabase Bucket
            const fileName = `ai-gen-${Date.now()}.jpg`;
            const filePath = `images/${fileName}`;

            const { error: uploadError } = await db.storage.from('recipe-images').upload(filePath, blob, {
