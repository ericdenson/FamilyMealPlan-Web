...
const adminApp = {
    async init() {
        ...
        let currentModule = '';
        if (path.includes('admin-influencers')) currentModule = 'users';
        else if (path.includes('admin-portal')) currentModule = 'dashboard';
        ...
        // Kick off constant badge polling
            ...
            try {
                const { data: ticketData, error } = await db.from('support_tickets')
                    .select(`
                        ...
                        status, 
                        subject_tag,
                        support_messages ( sender_type, timestamp )
                    `)
                    .eq('status', 'open')
                    .neq('subject_tag', 'RECIPE');

                if (error) return;

                const allTix = ticketData || [];
                const openCommsCount = allTix.filter(t => {
                    if (t.support_messages && t.support_messages.length > 0) {
                        const sortedMsgs = [...t.support_messages].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                        return sortedMsgs[0].sender_type === 'user';
                    }
                    return true;
                }).length;
                ...
                // Keep Dashboard KPI in sync if we're on the dashboard
                const portalUnreadTickets = document.getElementById('portal-unread-tickets');
                if (portalUnreadTickets) {
                    portalUnreadTickets.innerText = openCommsCount.toLocaleString('en-US');
                }
            } catch (e) {
    ...
    // --- GLOBAL APP VERSION CONTROLLER ---
    async loadAppVersions() {
        try {
            const { data, error } = await db.from('global_settings').select('key_value').eq('key_name', 'app_versions').single();
            if (error && error.code !== 'PGRST116') throw error;
            if (data && data.key_value) {
                const p = data.key_value;
                document.getElementById('cfg-app-min-version').value = p.minimum_required || '';
                document.getElementById('cfg-app-ios-url').value = p.store_url_ios || '';
                document.getElementById('cfg-app-android-url').value = p.store_url_android || '';
            }
        } catch (e) {
            console.warn("Failed to load App Versions: ", e);
        }
    },

    async saveAppVersions() {
        try {
            const minV = document.getElementById('cfg-app-min-version').value || '1.0.0';
            const iosU = document.getElementById('cfg-app-ios-url').value || '';
            const andU = document.getElementById('cfg-app-android-url').value || '';

            const payload = {
                minimum_required: minV,
                store_url_ios: iosU,
                store_url_android: andU
            };
            
            const { error } = await db.from('global_settings').upsert({ key_name: 'app_versions', key_value: payload });
            if (error) throw error;
            
            await this.logAdminAudit(`Updated Mobile Firewall (Min: ${minV})`, 'global');
            alert("App Lockout Policy activated! The mobile apps will query this on next boot.");
        } catch (e) {
            console.error(e);
            alert("Failed to save App Versions: " + e.message);
        }
    },
    ...
    async loadSubscriptionPricing() {
        try {
            ...
            if (data && data.key_value) {
                ...
                if (p.trial_duration_days) {
                    document.getElementById('cfg-sub-trial-days').value = p.trial_duration_days;
                }
                if (p.vip_trial_bonus_days) {
                    document.getElementById('cfg-sub-vip-bonus-days').value = p.vip_trial_bonus_days;
                }
            }
        } catch (e) {
    ...
    async saveSubscriptionPricing() {
        try {
            ...
            const trial = parseInt(document.getElementById('cfg-sub-trial-days').value) || 14;
            const vipBonus = parseInt(document.getElementById('cfg-sub-vip-bonus-days').value) || 16;

            const payload = {
                monthly_tiers: [m1, m2, m3],
                yearly_price: yr,
                trial_duration_days: trial,
                vip_trial_bonus_days: vipBonus
            };
    ...
    async fetchModerationQueue() {
        try {
            ...
            const { data, error } = await db.from('recipes')
                .select('id, title, author_id, visibility, instructions, is_duplicate, profanity_detected, duplicate_of_title, recipe_ingredients(ingredient_name), users!recipes_author_id_fkey(first_name, email)')
            ...
            tbody.innerHTML = data.map(r => {
                ...
                let flags = '';
                if (r.profanity_detected) flags += `<span style="background:#ffebee; color:#c62828; padding:2px 6px; border-radius:4px; font-size:0.75rem; font-weight:700; margin-left:8px; border:1px solid #ef9a9a;">PROFANITY</span>`;
                if (r.is_duplicate) flags += `<span style="background:#fff8e1; color:#f57f17; padding:2px 6px; border-radius:4px; font-size:0.75rem; font-weight:700; margin-left:8px; border:1px solid #ffe082;">DUPLICATE</span>`;

                return `
                <tr>
                    <td style="font-weight: 600;">${r.title} ${flags}</td>
    ...
    previewModerationRecipe(recipeId) {
        ...
        const authorEmail = recipe.users && recipe.users.email ? recipe.users.email : 'Unknown Email';

        let autoMessage = "";
        if (recipe.profanity_detected) {
            autoMessage = `Sorry, but the kind of language in your recipe, ${safeTitle}, will not be accepted in our recipe database.`;
        } else if (recipe.is_duplicate) {
            autoMessage = `Thank you soooo much for submitting your ${safeTitle} recipe... unfortunately we already have this recipe. It is called - ${recipe.duplicate_of_title || 'an existing recipe'}.`;
        }
        ...
        dialog.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 24px;">
                <div>
                  ...
                  <p style="margin:0; font-size:0.9rem; color:#888;">Submitted by: <strong style="color:#333;">${authorName}</strong> (${authorEmail})</p>
                </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea id="mod-message" rows="3" placeholder="Type a message to send directly to their notification tray..." style="width:100%; border-radius:12px; border:1px solid #ddd; padding:12px; font-family:inherit; font-size:0.95rem; resize:none;">${autoMessage}</textarea>
            </div>
    ...
    async executeModerationReject(recipeId, authorId, title, rawMessage) {
        ...
        try {
            ...
            alert("Recipe Rejected and returned to private cookbook.");
            await this.fetchModerationQueue();
            await this.fetchAdminRecipes();
        } catch (e) {
    ...
    async fetchAdminRecipes() {
        try {
            ...
            // Fetch historical saves from past_plans
            const { data: pastPlansData } = await db.from('past_plans').select('plan_snapshot');
            const pastTitleCounts = {};
            const pastIdCounts = {};
            if (pastPlansData) {
                pastPlansData.forEach(row => {
                    if (row.plan_snapshot && Array.isArray(row.plan_snapshot.details)) {
                        row.plan_snapshot.details.forEach(meal => {
                            if (meal.recipe_id) {
                                pastIdCounts[meal.recipe_id] = (pastIdCounts[meal.recipe_id] || 0) + 1;
                            } else if (meal.label) {
                                pastTitleCounts[meal.label] = (pastTitleCounts[meal.label] || 0) + 1;
                            }
                        });
                    }
                });
            ...
            adminState.recipes = rawRecipes.map(r => {
                ...
                return {
                    ...
                    lifetimeCooked: (scheduleCounts[r.id] || 0) + (pastIdCounts[r.id] || 0) + (pastTitleCounts[r.title] || 0)
                };
            });
    ...
    applyFilters() {
        ...
        adminState.filteredRecipes = adminState.recipes.filter(r => {
            ...
            if (stQ === 'pending_approval' && r.visibility !== 'pending_approval') return false;
        ...
        if (sortInput) {
            if (sortInput.value === 'placeholder_first') {
                adminState.filteredRecipes.sort((a,b) => {
                    const aIsPlaceholder = a.image && a.image.includes('loremflickr.com') ? 1 : 0;
                    const bIsPlaceholder = b.image && b.image.includes('loremflickr.com') ? 1 : 0;
                    if (aIsPlaceholder !== bIsPlaceholder) {
                        return bIsPlaceholder - aIsPlaceholder; // 1 comes before 0
                    }
                    // Secondary sort by newest
                    return new Date(b.created_at || 0) - new Date(a.created_at || 0);
                });
            } else if (sortInput.value === 'updated') {
                adminState.filteredRecipes.sort((a,b) => new Date(b.updated_at || 0) - new Date(a.updated_at || 0));
            } else if (sortInput.value === 'popular') {
                adminState.filteredRecipes.sort((a,b) => (b.lifetimeCooked || 0) - (a.lifetimeCooked || 0));
            } else if (sortInput.value === 'user_email') {
                adminState.filteredRecipes.sort((a,b) => {
                    const emailA = a.authorEmail || '';
                    const emailB = b.authorEmail || '';
                    
                    // Group recipes with no email at the end
                    if (emailA && !emailB) return -1;
                    if (!emailA && emailB) return 1;
                    
                    // If both have emails, sort alphabetically by email
                    if (emailA && emailB) {
                        const emailComp = emailA.localeCompare(emailB);
                        if (emailComp !== 0) return emailComp;
                    }
                    
                    // Secondary sort: Last Update Date (descending)
                    return new Date(b.updated_at || 0) - new Date(a.updated_at || 0);
                });
            } else if (sortInput.value === 'time_desc') {
                adminState.filteredRecipes.sort((a,b) => (b.time || 0) - (a.time || 0));
            }
        }
                ...
                <td>
                    <div class="recipe-thumb">
                        ...
                        <div>
                            ...
                            <span class="recipe-meta">${recipe.ingredients.length} Ingredients &nbsp; &nbsp; ${recipe.time} mins ${recipe.updated_at ? `&nbsp; &nbsp; <i class="fa-regular fa-clock"></i> ${new Date(recipe.updated_at).toLocaleDateString()}` : ''} ${recipe.authorEmail ? (recipe.visibility === 'private' ? `&nbsp; &nbsp; <span style="color:#e57373;"><i class="fa-solid fa-lock"></i> Private: ${recipe.authorEmail}</span>` : (recipe.visibility === 'pending_approval' ? `&nbsp; &nbsp; <span style="color:#ef6c00; font-weight:700;"><i class="fa-solid fa-hourglass-half"></i> Pending Approval: ${recipe.authorEmail}</span>` : `&nbsp; &nbsp; <span style="color:#d4af37; font-weight:700;"><i class="fa-solid fa-globe"></i> Community: ${recipe.authorEmail}</span>`)) : ''}</span>
                        </div>
                    </div>
                </td>
    ...
    openRecipeEditor(id) {
        ...
        const proteinEl = document.getElementById('edit-protein');
        const carbsEl = document.getElementById('edit-carbs');
        const fatEl = document.getElementById('edit-fat');
        const fiberEl = document.getElementById('edit-fiber');
        if (proteinEl) proteinEl.value = recipe.protein !== undefined ? recipe.protein : "";
        if (carbsEl) carbsEl.value = recipe.carbs !== undefined ? recipe.carbs : "";
        if (fatEl) fatEl.value = recipe.fat !== undefined ? recipe.fat : "";
        if (fiberEl) fiberEl.value = recipe.fiber !== undefined ? recipe.fiber : "";
        ...
        if (lEl) lEl.innerText = recipe.lifetimeCooked.toLocaleString();
    ...
    async saveRecipe() {
        ...
        try {
            ...
            const fatEl = document.getElementById('edit-fat');
            const fiberEl = document.getElementById('edit-fiber');
            const protein_grams = proteinEl ? (proteinEl.value === "" ? null : parseInt(proteinEl.value) || 0) : null;
            const carbs_grams = carbsEl ? (carbsEl.value === "" ? null : parseInt(carbsEl.value) || 0) : null;
            const fat_grams = fatEl ? (fatEl.value === "" ? null : parseInt(fatEl.value) || 0) : null;
            const fiber_grams = fiberEl ? (fiberEl.value === "" ? null : parseInt(fiberEl.value) || 0) : null;
    ...
    async fetchAdminUsers() {
        try {
            ...
            adminState.users = data.map(u => ({
                ...u,
                first_name: u.first_name || 'Family Chef',
                last_name: u.last_name || '',
                subscription_status: u.is_subscribed ? 'active' : (u.subscription_status || 'free'),
                is_lifetime_free: u.is_lifetime_free || false,
    ...
    applyUserFilters() {
        ...
        adminState.filteredUsers = adminState.users.filter(u => {
            const matchSearch = String(u.id).toLowerCase().includes(sQ) ||
                ...
                (u.last_name && u.last_name.toLowerCase().includes(sQ));
            if (!matchSearch) return false;

            if (stQ === 'active' && (u.subscription_status !== 'active' || u.is_lifetime_free)) return false;
            if (stQ === 'trial' && (u.subscription_status !== 'trial' || u.is_lifetime_free)) return false;
            if (stQ === 'lifetime' && !u.is_lifetime_free) return false;
            if (stQ === 'canceled' && (!['canceled', 'expired'].includes(u.subscription_status) || u.is_lifetime_free)) return false;
    ...
    renderAdminUsersTable() {
        ...
        adminState.filteredUsers.forEach(user => {
            ...
            if (user.is_lifetime_free) {
                badgeConfig = { bg: '#f3e5f5', color: '#7e57c2', text: 'LifeTime' };
            } else if (user.is_subscribed || user.subscription_status === 'active') {
            ...
            const initials = user.last_name ? user.first_name.charAt(0).toUpperCase() + user.last_name.charAt(0).toUpperCase() : user.first_name.charAt(0).toUpperCase();
            ...
            tr.innerHTML = `
                <td>
                    <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <span style="font-weight:600; color:var(--text-main); font-size:0.85rem;">${fullName}</span>
                    </div>
                </td>
    ...
    openUserPanel(id) {
        ...
        const initials = user.last_name ? user.first_name.charAt(0).toUpperCase() + user.last_name.charAt(0).toUpperCase() : user.first_name.charAt(0).toUpperCase();
        const fullName = user.last_name ? `${user.first_name} ${user.last_name}` : user.first_name;
        document.getElementById('au-initials').innerText = initials;
        document.getElementById('au-name').innerText = fullName;
    ...
    async fetchAdminTickets() {
        try {
            ...
            const openCommsCount = allTix.filter(t => {
                ...
                if (t.subject_tag === 'RECIPE') return false;
                if (t.support_messages && t.support_messages.length > 0) {
                    const sortedMsgs = [...t.support_messages].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                    return sortedMsgs[0].sender_type === 'user';
                }
    ...
    renderCommsList() {
        ...
        targetArray.forEach(item => {
            ...
            if (adminState.commsTab === 'global') {
                ...
                if (item.users) {
                    const u = item.users;
                    const fullName = u.last_name ? `${u.first_name} ${u.last_name}` : (u.first_name || 'Family Chef');
                    name = `${fullName} <span style="font-weight: normal; font-size: 0.85em; color: #888;">(${u.email || 'No Email'})</span>`;
                } else {
                    name = 'Unknown User';
                }
                dateStr = new Date(item.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
            ...
            card.innerHTML = `
                <div class="msg-header">
                    ...
                    <span class="msg-time">${dateStr} &nbsp;&nbsp; <i class="fa-solid fa-trash-can" style="color:#e53935; cursor:pointer;" onclick="event.stopPropagation(); adminApp.deleteCommunication('${item.id}', '${adminState.commsTab === 'global' ? 'broadcast' : 'ticket'}');" title="Delete Thread"></i></span>
                </div>
    ...
    renderThreadHistory(item) {
        ...
        if (adminState.commsTab === 'global') {
            ...
            // It's a User Thread
            let thTitle = 'Unknown User';
            if (item.users) {
                const u = item.users;
                const fullName = u.last_name ? `${u.first_name} ${u.last_name}` : (u.first_name || 'Family Chef');
                thTitle = `${fullName} (${u.email || 'No Email'})`;
            }
            document.getElementById('th-title').innerText = thTitle;
            ...
            thClose.style.display = (item.status === 'closed' || item.subject_tag === 'RECIPE') ? 'none' : 'block';
    ...
    async deleteCommunication(id, type) {
        if (!confirm("Are you sure you want to remove this thread from the dashboard?")) return;
        try {
            if (type === 'broadcast') {
                const { error } = await db.from('inbox_messages').delete().eq('id', id);
                if (error) throw error;
                await this.logAdminAudit('Hard Deleted Global Broadcast', id);
            } else if (type === 'ticket') {
                const { error } = await db.from('support_tickets').update({ status: 'deleted' }).eq('id', id);
                if (error) throw error;
                await this.logAdminAudit('Soft Deleted Support Ticket', id);
            }
            if (adminState.activeTicketId === id) {
                adminState.activeTicketId = null;
                const tm = document.getElementById('thread-messages');
                if (tm) tm.innerHTML = '<div style="padding:40px; text-align:center; color:var(--text-muted); font-weight:500;">Communication thread has been removed.</div>';
                const ti = document.getElementById('th-info');
            ...
            alert("Error deleting communication: " + e.message);
        }
    }, // END COMMUNICATIONS
    ...
    async computePortalDashboard() {
        try {
            ...
            // ALWAYS fetch fresh tickets for the dashboard KPI to prevent stale cache
            const { data: freshTickets } = await db.from('support_tickets')
                .select('status, subject_tag, support_messages(sender_type, timestamp)')
                .eq('status', 'open')
                .neq('subject_tag', 'RECIPE');
            const openTickets = freshTickets ? freshTickets.filter(t => {
                if (t.support_messages && t.support_messages.length > 0) {
                    const sortedMsgs = [...t.support_messages].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                    return sortedMsgs[0].sender_type === 'user';
                }
            ...
            if (usersTbody) {
                ...
                recentSignups.forEach(u => {
                    ...
                    if (u.is_lifetime_free) {
                        statusHtml = '<span style="background:#f3e5f5; color:#7e57c2; padding:4px 8px; border-radius:4px; font-size:0.75rem; font-weight:700;">LIFETIME</span>';
                    } else if (u.is_subscribed || u.subscription_status === 'active') {
                    ...
                    const initials = u.last_name ? (u.first_name || 'A').charAt(0).toUpperCase() + u.last_name.charAt(0).toUpperCase() : (u.first_name || 'A').charAt(0).toUpperCase();
                    const fullName = u.last_name ? `${u.first_name} ${u.last_name}` : (u.first_name || 'Family Chef');
                    tr.innerHTML = `
                        <td>
                            <div style="display:flex; align-items:center; gap:6px;">
                                <div style="width:20px; height:20px; border-radius:50%; background:var(--accent-color); color:white; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.6rem;">${initials}</div>
                                <span style="font-weight:600; color:var(--text-main); font-size:0.8rem;">${fullName}</span>
                            </div>
                        </td>
    ...
    populateRecipeDietDropdowns() {
        ...
        // Get all diets for Admin assignment, including shadow categories like snacks
        const activeDiets = adminState.globalDiets;
    ...
    async calculateCalories() {
        ...
        try {
            ...
            if (data && data.calories) {
                ...
                const fatEl = document.getElementById('edit-fat');
                const fiberEl = document.getElementById('edit-fiber');
                if (proteinEl) proteinEl.value = data.protein !== undefined ? data.protein : "";
                if (carbsEl) carbsEl.value = data.carbs !== undefined ? data.carbs : "";
                if (fatEl) fatEl.value = data.fat !== undefined ? data.fat : "";
                if (fiberEl) fiberEl.value = data.fiber !== undefined ? data.fiber : "";
    ...
    async convertToImperial() {
        ...
        if (ingredients.length === 0 && instructions.length === 0) {
            return alert("You must add ingredients or instructions first!");
        }

        const btn = document.getElementById('ai-imperial-btn');
        const oldHtml = btn.innerHTML;
        ...
        try {
            const { data, error } = await db.functions.invoke('magic-import', {
                body: { type: 'convert_imperial', payloads: { ingredients, instructions } }
            });
            ...
            console.error(e);
            alert("AI Conversion Failed: " + e.message);
            btn.innerHTML = oldHtml;
        } finally {
            btn.style.pointerEvents = 'all';
        }
