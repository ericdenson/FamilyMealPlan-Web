    ...
    async generateNewImage() {
        ...
        try {
            ...
            const response = await fetch("https://fqhyzrfaacnxqrbznmwo.supabase.co/functions/v1/generate-image", {
                ...
                body: JSON.stringify({ title: title, id: adminState?.activeRecipeId || null })
            });
            ...
            // 2. Extract final URL from the Edge function (which already handled Supabase storage upload or cache delivery!)
            const data = await response.json();
            
            if (data.cached) {
                alert("Cost Savings: Found an existing visual from a highly similar recipe! To force a new AI generation, please change the recipe title temporarily.");
            }
